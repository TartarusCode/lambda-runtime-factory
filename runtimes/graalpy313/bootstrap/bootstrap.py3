...
logging.basicConfig(
    level=os.getenv("GRAALPY_RUNTIME_LOG_LEVEL", "INFO").upper(),
...
RUNTIME_CLIENT_TIMEOUT = float(os.getenv("GRAALPY_RUNTIME_API_TIMEOUT", "310"))
POLL_RETRY_BACKOFF_SECONDS = float(os.getenv("GRAALPY_RUNTIME_POLL_BACKOFF", "0.25"))
...
for path in (
    "/opt/graalpy",
    "/opt/graalpy/lib/python3.13/site-packages",
...
try:
    from lambda_runtime_graalpy.init import run_configured_init_hooks
    from lambda_runtime_graalpy.logging import clear_context, set_context
    from lambda_runtime_graalpy.tracing import set_trace_id
except ImportError:
...
class RuntimeApiClient:
    """Persistent client for the Lambda Runtime API."""
...
def main() -> None:
    ...
    logger.info("Starting GraalPy Lambda runtime")
    ...
    logger.info("GraalPy Lambda runtime shutdown complete")
